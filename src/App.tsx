  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 flex flex-col items-center">
      ...
      <main className="w-full max-w-md relative">
        <div className="relative h-[680px] w-full perspective-1000">
          <motion.div 
          ...
          >
            ...
            {/* Back Side */}
            <div className="absolute inset-0 bg-white rounded-[3rem] p-8 shadow-2xl backface-hidden rotate-y-180 flex flex-col border-4 border-white cursor-pointer overflow-hidden">
              <div className="h-40 rounded-[2.5rem] overflow-hidden mb-6 shadow-inner border border-slate-100 shrink-0">
                <img src={currentWord.image} className="w-full h-full object-cover" alt="visual" referrerPolicy="no-referrer" />
              </div>
              
              <div className="text-center mb-6 shrink-0">
                <div className="flex flex-col items-center gap-1">
                  <span className="text-5xl font-black text-red-600 font-jp mb-1 drop-shadow-sm">{currentWord.kanji}</span>
                  <span className="text-xl font-bold text-slate-400 font-jp mb-1">{currentWord.kana}</span>
                  <h2 className="text-3xl font-black text-slate-800 tracking-tight leading-tight">{currentWord.spanish}</h2>
                  <span className="text-slate-400 font-black tracking-widest uppercase text-[10px] mt-1">{currentWord.romaji}</span>
                </div>
              </div>

              <div className="flex flex-col gap-4 overflow-y-auto no-scrollbar pb-4 flex-1">
                {/* Fonética Kana */}
                <div className="bg-slate-50 p-5 rounded-[2rem] border border-slate-100 shrink-0">
                  <div className="flex items-center gap-2 mb-3 text-slate-400">
                    <Music size={16} className="text-blue-400" />
                    <span className="text-[10px] font-black uppercase tracking-widest">Fonética Kana</span>
                  </div>
                  <div className="flex flex-wrap gap-2.5">
                    {currentWord.kanaBreakdown.map((k, i) => (
                      <div key={i} className="flex flex-col items-center bg-white px-3.5 py-2 rounded-xl shadow-sm border border-slate-100 min-w-[44px]">
                        <span className="text-xl font-black text-slate-800 font-jp leading-none mb-1">{k.char}</span>
                        <span className="text-[10px] font-black text-slate-400 uppercase">{k.sound}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {currentWord.kanjiBreakdown.length > 0 && (
                  <div className="bg-slate-50 p-5 rounded-[2rem] border border-slate-100 shrink-0">
                    <div className="flex items-center gap-2 mb-3 text-slate-400">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-widest">Desglose de Kanji</span>
                    </div>
                    <div className="flex flex-wrap gap-2.5">
                      {currentWord.kanjiBreakdown.map((k, i) => (
                        <div key={i} className="flex flex-col items-center bg-white px-3.5 py-2 rounded-xl shadow-sm border border-slate-100 min-w-[50px]">
                          <span className="text-xl font-black text-red-600 font-jp leading-none mb-1">{k.char}</span>
                          <span className="text-[8px] font-black text-blue-500 uppercase leading-none mb-0.5">{k.reading}</span>
                          <span className="text-[9px] font-bold text-slate-700 text-center leading-tight">{k.mean}</span>
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        <div className="flex gap-3 mt-4">
          <button 
            ...
            className="flex-1 bg-white p-4 rounded-3xl border border-slate-200 text-slate-300 active:bg-slate-100 disabled:opacity-20 transition-all flex items-center justify-center"
          >
          ...
          <button 
            ...
            className={`flex-[2] flex items-center justify-center gap-2 p-4 rounded-3xl font-black text-xs uppercase transition-all
              ${masteredIds.includes(currentWord.id) 
                ? 'bg-green-500 text-white shadow-lg shadow-green-100' 
                : 'bg-white text-slate-200 border-2 border-dashed border-slate-200'}`}
          >
            <CheckCircle size={20} fill={masteredIds.includes(currentWord.id) ? "white" : "none"} />
          ...
          <button 
            ...
            className="flex-1 bg-white p-4 rounded-3xl border border-slate-200 text-slate-300 active:bg-slate-100 transition-all flex items-center justify-center"
          >
